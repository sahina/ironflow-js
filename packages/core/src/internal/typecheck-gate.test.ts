/**
 * Drift-detection guard for issue #642.
 *
 ...
 *
 * Walks the whole package root (excluding `node_modules`/`dist`) so the
 * gate stays valid even if test files migrate to `tests/`,
 * `__type_tests__/`, or new top-level locations later.
 *
...
describe("typecheck gate (issue #642)", () => {
